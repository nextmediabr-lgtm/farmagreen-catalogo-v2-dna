...
export async function catalogV67(): Promise<Catalog> {
  if (cache) return cache;
  const parsed = JSON.parse(await fs.readFile(path.join(ROOT, "data", "catalog-v67.json"), "utf8")) as Catalog;
  if (!Array.isArray(parsed.products)) throw new Error("Catálogo V6.7 inválido.");
  cache = { ...parsed, totalProducts: parsed.products.length };
  return cache;
}

export async function productV67(id: string) {
  return (await catalogV67()).products.find((product) => product.slug === id || product.publicId === id);
}

export async function similarV67(product: Product) {
  return (await catalogV67()).products
    .filter((candidate) => candidate.publicId !== product.publicId)
    .map((candidate) => ({
      candidate,
      score:
        (candidate.brand.slug === product.brand.slug ? 5 : 0) +
        candidate.categorySlugs.filter((category) => product.categorySlugs.includes(category)).length * 3 +
        candidate.needs.filter((need) => product.needs.includes(need)).length * 2 +
...
export function catalogPageV67(catalog: Catalog, query = new URLSearchParams()) {
  ...
  const brandStats = brands.map((brand) => ({
    brand,
    count: catalog.products.filter((product) => product.brand.name === brand).length,
    best: offers.find((product) => product.brand.name === brand)?.discountPercent || 0,
  }));
  const initialNeedLabel = context.need === "Todas" ? "Todas" : NEED_LABELS.get(context.need) || "Todas";
  const initialBrandStat = brandStats.find((item) => item.brand === context.brand);
  const initialBrandLabel =
    context.brand === "Todas" ? `Todas · ${catalog.totalProducts}` : `${context.brand} · ${initialBrandStat?.count || 0}`;

  return shell67(
    context.metaTitle,
    context.metaDescription,
    `
<section class="v65-panel v65-search-panel v65-top-search v67-discovery" id="buscar-v67">
  <div class="v67-primary-row">
    ...
    <form class="v66-search v67-search" role="search">
      <label class="v67-visually-hidden" for="searchV67">Producto, marca o necesidad</label>
      <span class="v67-search-icon">${searchIcon()}</span>
      <input id="searchV67" type="search" placeholder="Producto, marca o necesidad" autocomplete="off" spellcheck="false">
    </form>
    <button class="v65-link-button" id="clearFiltersV67" type="button">Limpiar</button>
  </div>
  <div class="v67-filter-grid" aria-label="Filtros del catálogo">
    <div class="v67-filter-menu v67-need-menu" data-filter-menu="need">
      <button class="v67-menu-trigger" type="button" data-filter-menu-trigger="need" aria-expanded="false" aria-controls="needMenuV67">
        <span class="v67-menu-label">¿Qué necesitás?</span>
        <strong id="needSummaryV67">${e(initialNeedLabel)}</strong>
        ${chevronIcon()}
      </button>
      <div class="v67-menu-popover v67-need-popover" id="needMenuV67" data-filter-menu-popover aria-hidden="true" aria-label="Elegir necesidad" inert>
    ...
    <div class="v67-filter-menu v67-brand-menu" data-filter-menu="brand" id="marcas-v67">
      <button class="v67-menu-trigger" type="button" data-filter-menu-trigger="brand" aria-expanded="false" aria-controls="brandMenuV67">
        <span class="v67-menu-label">Marca</span>
        <strong id="brandSummaryV67">${e(initialBrandLabel)}</strong>
        ${chevronIcon()}
      </button>
      <div class="v67-menu-popover v67-brand-popover" id="brandMenuV67" data-filter-menu-popover aria-hidden="true" aria-label="Elegir marca" inert>
...
<section class="v65-products" id="productos-v67">
  <div class="v65-head v66-catalog-head">
    <div>
      <p class="v65-k" id="modeV67">${e(context.mode)}</p>
      <h1 id="catalogTitleV67">${e(context.title)}</h1>
      <p class="v66-context" id="contextV67">${e(context.copy)}</p>
    </div>
    <div class="v66-catalog-tools">
      <p id="countV67" aria-live="polite"></p>
      <button class="v65-link-button" type="button" id="showAllV67">Ver todo el catálogo</button>
    </div>
  </div>
  <section class="v65-grid" id="gridV67">${initial.slice(0, 24).map((product) => cardV67(product)).join("")}</section>
  <div class="morebox"><button id="loadMoreV67" type="button" aria-label="Cargar más productos">Cargar más productos</button></div>
</section>

<script>window.__FG67=${json({ base: BASE, products: catalog.products, context: context.state })}</script>`,
    {
      bodyClass: "v65 v66 v67",
      canonicalPath: "/catalogo-v6-7/",
      ogType: "website",
      ogImage: context.ogImage,
      homeHref: "/catalogo-v6-7/",
      links: [
        { href: "/catalogo-v6-7/#productos-v67", label: "Ofertas", nav: "ofertas" },
        { href: "/catalogo-v6-7/#marcas-v67", label: "Marcas", nav: "marcas" },
        { href: "/catalogo-v6-7/#buscar-v67", label: "Buscar", nav: "buscar" },
        { href: "/catalogo-v6-7/?scope=todo#productos-v67", label: "Productos", nav: "productos" },
      ],
    },
  );
}

export function productPageV67(product: Product, related: Product[]) {
  const discount = Math.round(product.discountPercent || 0);
  return shell67(
    `${product.name} | Farmagreen Rosario`,
    product.description.slice(0, 155),
    `
<nav class="crumb v65-crumb"><a href="${u("/catalogo-v6-7/#productos-v67")}">Volver al catálogo</a><span>/</span><span>${e(product.brand.name)}</span></nav>
<article class="pdp v65-pdp v66-pdp">
  <div class="v66-card-top v67-pdp-card-top">
    <p class="v66-brand">${e(product.brand.name)}</p>
    ${discount > 0 ? `<span class="v66-discount">${discount}%</span>` : ""}
  </div>
  <div class="photo v65-photo"><img src="${e(product.images.detail)}" alt="${e(product.name)}"></div>
  <div class="buybox v65-buybox">
    ...
    <dl class="v66-pdp-facts">
      ...
      <div><dt>Uso principal</dt><dd>${e(usage(product))}</dd></div>
    </dl>
    ${priceDetail(product)}
    <a class="cta" href="${wa(`Hola Farmagreen Rosario, quiero consultar por ${product.brand.name} - ${product.name}. Link: ${u(`/producto-v6-7/${product.slug}/`)}`)}">Consultar este producto por WhatsApp</a>
    <ul class="v65-service-list">
      <li>Consulta humana y directa.</li>
      <li>Retiro coordinado en Rosario.</li>
      <li>Podés compartir esta URL tal como está.</li>
    </ul>
    <section><h2>Detalle</h2><p>${e(product.description)}</p></section>
  </div>
</article>
<section class="v65-products v65-related">
  ...
  <section class="v65-grid">${related.map((item) => cardV67(item)).join("")}</section>
</section>
<script type="application/ld+json">${json({
    ...
    {
      bodyClass: "v65 v66 v67 product-detail",
      canonicalPath: `/producto-v6-7/${product.slug}/`,
      ogType: "product",
      ogImage: product.images.detail,
      homeHref: "/catalogo-v6-7/",
      links: [{ href: "/catalogo-v6-7/#productos-v67", label: "Catálogo" }],
    },
  );
}

export function notFoundPageV67() {
  return shell67(
    "No encontrado | Farmagreen Rosario",
    "Producto no encontrado",
    `<section class="empty"><h1>No encontramos ese producto</h1><a href="${u("/catalogo-v6-7/")}">Volver al catálogo</a></section>`,
    { bodyClass: "v65 v66 v67", homeHref: "/catalogo-v6-7/" },
  );
}

type QueryState = { q: string; brand: string; need: string; scope: "ofertas" | "todo" };
...
function pageContext(catalog: Catalog, query: URLSearchParams): PageContext {
  const availableBrands = new Set(catalog.products.map((product) => product.brand.name));
  const requestedBrand = query.get("marca") || "";
  const brand = availableBrands.has(requestedBrand) ? requestedBrand : "Todas";
  const requestedNeed = query.get("need") || "";
  const need = NEED_LABELS.has(requestedNeed) ? requestedNeed : "Todas";
  const q = String(query.get("q") || "").trim();
  const scope: "ofertas" | "todo" = query.get("scope") === "todo" || q || brand !== "Todas" || need !== "Todas" ? "todo" : "ofertas";
  const state = { q, brand, need, scope };
  let mode = "Ofertas";
  ...
  if (q) {
    ...
    mode = "Selección";
    title = brand;
    copy = `${NEED_LABELS.get(need) || need} de ${brand}.`;
  } else if (brand !== "Todas") {
    mode = "Marca";
    title = brand;
  ...
  const metaTitle = brand !== "Todas" ? `${brand} | Catálogo Farmagreen V6.7` : need !== "Todas" ? `${title} | Catálogo Farmagreen V6.7` : q ? `${title} | Farmagreen` : "Farmagreen Rosario | Catálogo V6.7";
  ...
  return { ...state, state, mode, title, copy, metaTitle, metaDescription, ogImage: contextual ? filtered[0]?.images.card || SOCIAL_IMAGE : SOCIAL_IMAGE };
}
...
function filteredProducts(products: Product[], state: QueryState) {
  ...
  return products
    ...
    .filter((product) => state.brand === "Todas" || product.brand.name === state.brand)
    .filter((product) => state.need === "Todas" || product.needs.includes(state.need))
    .filter((product) => {
      if (!terms.length) return true;
      const text = normalize([product.name, product.brand.name, product.line, ...product.aliases].join(" "));
      return terms.every((term) => text.includes(term));
    })
    .sort((left, right) => right.discountPercent - left.discountPercent || right.savingAmount - left.savingAmount || left.name.localeCompare(right.name, "es"));
}
...
function shell67(title: string, description: string, body: string, options: ShellOptions = {}) {
  const homeHref = options.homeHref || "/catalogo-v6-7/";
  const links = options.links || [];
  const canonical = options.canonicalPath ? `<link rel="canonical" href="${u(options.canonicalPath)}">` : "";
  const og = `<meta property="og:type" content="${e(options.ogType || "website")}"><meta property="og:title" content="${e(title)}"><meta property="og:description" content="${e(description)}"><meta property="og:site_name" content="Farmagreen Rosario"><meta property="og:locale" content="es_AR">${options.ogImage ? `<meta property="og:image" content="${e(options.ogImage)}">` : ""}<meta name="twitter:card" content="${options.ogImage ? "summary_large_image" : "summary"}">`;
  return `<!doctype html><html lang="es-AR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${e(title)}</title><meta name="description" content="${e(description)}">${canonical}${og}<link rel="icon" href="${u("/logo_farmagreen.png")}"><link rel="stylesheet" href="${u("/styles-v6-5.css")}"><link rel="stylesheet" href="${u("/styles-v6-6.css")}"><link rel="stylesheet" href="${u("/styles-v6-7.css")}"></head><body${options.bodyClass ? ` class="${e(options.bodyClass)}"` : ""}><header class="top"><a href="${u(homeHref)}" class="brandmark"><img src="${u("/logo_farmagreen.png")}" alt="Farmagreen"></a><div class="toplinks">${links.map((link) => `<a href="${u(link.href)}"${link.nav ? ` data-nav="${e(link.nav)}"` : ""}>${e(link.label)}</a>`).join("")}</div><a class="topwa" href="${wa("Hola Farmagreen Rosario, quiero consultar.")}" aria-label="Abrir WhatsApp de Farmagreen">${waIcon()}<span>WhatsApp</span></a></header><main>${body}</main><a class="float" href="${wa("Hola Farmagreen Rosario, quiero hacer una consulta.")}" aria-label="Consultar por WhatsApp">${waIcon()}</a><script type="module" src="${u("/app-v6-7.js")}"></script></body></html>`;
}

function cardV67(product: Product) {
  const discount = Math.round(product.discountPercent || 0);
  return `<article class="v66-card"><a class="v65-hit" href="${u(`/producto-v6-7/${product.slug}/`)}" aria-label="Ver ${e(product.name)}"></a><div class="v66-card-top"><p class="v66-brand">${e(product.brand.name)}</p>${discount > 0 ? `<span class="v66-discount">${discount}%</span>` : ""}</div><div class="v66-media"><img src="${e(product.images.card)}" alt="${e(product.name)}" loading="lazy" decoding="async"></div><div class="v66-card-body"><h3>${e(product.name)}</h3><dl class="v66-facts"><div><dt>Presentación</dt><dd>${e(presentation(product))}</dd></div><div><dt>Uso principal</dt><dd>${e(usage(product))}</dd></div></dl>${priceCard(product)}<a class="ask v66-ask" href="${wa(`Hola Farmagreen Rosario, quiero consultar por ${product.brand.name} - ${product.name}. Link: ${u(`/producto-v6-7/${product.slug}/`)}`)}">Consultar</a></div></article>`;
}
...
function priceCard(product: Product) {
...
function priceDetail(product: Product) {
...
function presentation(product: Product) {
  const matches = [...product.name.matchAll(/\b(\d+(?:[.,]\d+)?)\s*(ml|cc|cm3|g|gr|grs|kg|cápsulas?|caps?\.?|comprimidos?|tabletas?|sobres?|ampollas?|unidades?)\b/gi)];
...
function usage(product: Product) {
