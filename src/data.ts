...
export type ResponsiveImageSet={width:number;height:number;avif?:Record<string,string>;webp?:Record<string,string>;jpeg?:Record<string,string>};
...
function clean(p:Product):Product{return{publicId:t(p.publicId),slug:t(p.slug),name:t(p.name),brand:{id:t(p.brand?.id),slug:t(p.brand?.slug),name:t(p.brand?.name),aliases:Array.isArray(p.brand?.aliases)?p.brand.aliases.map(t):[]},line:t(p.line),primaryCategory:t(p.primaryCategory),categorySlugs:Array.isArray(p.categorySlugs)?p.categorySlugs.map(t):[],needs:Array.isArray(p.needs)?p.needs.map(t):[],aliases:Array.isArray(p.aliases)?p.aliases.map(t):[],description:t(p.description),listPrice:n(p.listPrice),offerPrice:n(p.offerPrice),savingAmount:n(p.savingAmount),discountPercent:n(p.discountPercent),images:{card:t(p.images?.card),detail:t(p.images?.detail||p.images?.card)}}}
