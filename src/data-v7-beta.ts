...
const DEFAULT_CATALOG = path.join(os.tmpdir(), "farmagreen-catalog-v7-beta.json");
...
export type CatalogV69 = Omit<Catalog, "products"> & {
  ...
  v7Beta?: {
    ...
    missingFacetSlugs?: string[];
    currentCycleCoverage?: number;
...
export async function catalogV69Data(environment: NodeJS.ProcessEnv = process.env): Promise<CatalogV69> {
  ...
  const catalogPath = environment.V7_BETA_CATALOG_FILE?.trim() || DEFAULT_CATALOG;
  const exclusionsPath = environment.V7_BETA_EXCLUSIONS_FILE?.trim() || DEFAULT_EXCLUSIONS;
  const key = [
    await fileFingerprint(catalogPath),
    await fileFingerprint(exclusionsPath),
    environment.V7_BETA_REQUIRE_EXCLUSIONS || "1",
  ].join("\n");
  ...
  if (!Array.isArray(parsed.products)) throw new Error("Snapshot V7 Beta Local inválido.");

  const exclusions = await loadExclusionsV69(exclusionsPath, environment.V7_BETA_REQUIRE_EXCLUSIONS !== "0");
...
export async function setCatalogV69Data(
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("Snapshot V7 Beta Local inválido.");
  }
  const exclusionsPath = environment.V7_BETA_EXCLUSIONS_FILE?.trim() || DEFAULT_EXCLUSIONS;
  const exclusions = await loadExclusionsV69(
    exclusionsPath,
    environment.V7_BETA_REQUIRE_EXCLUSIONS !== "0",
  );
...
function cleanProductV69(product: ProductV69): ProductV69 {
  ...
  return reviseFpsPrimaryUseV69({
    ...
    catalogFacets: cleanCatalogFacetsV69(product.catalogFacets),
    catalogPositions: cleanCatalogPositionsV69(product.catalogPositions),
    source: product.source && typeof product.source === "object" ? { ...product.source, url: cleanOptional(product.source.url) } : undefined,
  });
}

function cleanCatalogFacetsV69(value: unknown): CatalogFacetV69[] | undefined {
  if (!Array.isArray(value)) return undefined;
  const facets = new Map<string, CatalogFacetV69>();
  for (const raw of value) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) continue;
    const candidate = raw as Record<string, unknown>;
    const slug = cleanOptional(candidate.slug);
    const name = cleanOptional(candidate.name);
    if (!slug || !name) continue;
    facets.set(slug, {
      slug,
      name,
      aliases: Array.isArray(candidate.aliases)
        ? [...new Set(candidate.aliases.map(cleanOptional).filter((alias): alias is string => Boolean(alias)))]
        : [],
      kind: candidate.kind === "collection" ? "collection" : "brand",
    });
  }
  return facets.size ? [...facets.values()] : undefined;
}

function cleanCatalogPositionsV69(value: unknown): Record<string, number> | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const positions = Object.fromEntries(
    Object.entries(value as Record<string, unknown>)
      .map(([slug, raw]) => [cleanOptional(slug), Number(raw)] as const)
      .filter(([slug, position]) => Boolean(slug) && Number.isInteger(position) && position > 0),
  );
