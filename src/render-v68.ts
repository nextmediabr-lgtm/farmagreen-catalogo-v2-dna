...
export async function productV68(id: string) {
...
export function searchTextV68(product: Product) {
...
export function catalogPageV68(catalog: Catalog, query = new URLSearchParams(), origin = "http://127.0.0.1:8100") {
  ...
  return shell68(
    ...
    `
<section class="v65-panel v65-search-panel v65-top-search v67-discovery" id="buscar-v68">
  <div class="v67-primary-row">
    ...
    <form class="v66-search v67-search" role="search">
      <label class="v67-visually-hidden" for="searchV68">Producto, marca o necesidad</label>
      <span class="v67-search-icon">${searchIcon()}</span>
      <input id="searchV68" type="search" placeholder="Producto, marca o necesidad" autocomplete="off" spellcheck="false">
    </form>
    <button class="v65-link-button" id="clearFiltersV68" type="button">Limpiar</button>
  </div>
  <div class="v67-filter-grid" aria-label="Filtros del catálogo">
    <div class="v67-filter-menu v67-need-menu" data-filter-menu="need">
      <button class="v67-menu-trigger" type="button" data-filter-menu-trigger="need" aria-expanded="false" aria-controls="needMenuV68">
        <span class="v67-menu-label">¿Qué necesitás?</span>
        <strong id="needSummaryV68">${e(initialNeedLabel)}</strong>
        ${chevronIcon()}
      </button>
      <div class="v67-menu-popover v67-need-popover" id="needMenuV68" data-filter-menu-popover aria-hidden="true" aria-label="Elegir necesidad" inert>
    ...
    <div class="v67-filter-menu v67-brand-menu" data-filter-menu="brand" id="marcas-v68">
      <button class="v67-menu-trigger" type="button" data-filter-menu-trigger="brand" aria-expanded="false" aria-controls="brandMenuV68">
        <span class="v67-menu-label">Marca</span>
        <strong id="brandSummaryV68">${e(initialBrandLabel)}</strong>
        ${chevronIcon()}
      </button>
      <div class="v67-menu-popover v67-brand-popover" id="brandMenuV68" data-filter-menu-popover aria-hidden="true" aria-label="Elegir marca" inert>
...
<section class="v65-products" id="productos-v68">
  <div class="v65-head v66-catalog-head">
    ...
    <div class="v66-catalog-tools">
      ...
      <button class="v65-link-button" type="button" id="showAllV68">Ver todo el catálogo</button>
    </div>
  </div>
  <section class="v65-grid" id="gridV68">${initial.slice(0, 24).map((product) => cardV68(product, origin)).join("")}</section>
...
<script type="application/json" id="fg68-data">${json({ base: BASE, origin, products: publicCatalogV68(catalog).products, context: context.state })}</script>`,
    {
      ...
      origin,
      canonicalPath: "/catalogo-v6-8/",
      ogType: "website",
      ogImage: context.ogImage,
      homeHref: "/catalogo-v6-8/",
      links: [
        { href: "/catalogo-v6-8/#productos-v68", label: "Ofertas", nav: "ofertas" },
        { href: "/catalogo-v6-8/#marcas-v68", label: "Marcas", nav: "marcas" },
        { href: "/catalogo-v6-8/#buscar-v68", label: "Buscar", nav: "buscar" },
        { href: "/catalogo-v6-8/?scope=todo#productos-v68", label: "Productos", nav: "productos" },
      ],
    },
  );
}
...
export function productPageV68(product: Product, related: Product[], origin = "http://127.0.0.1:8100") {
  ...
  return shell68(
    ...
    `
<nav class="crumb v65-crumb"><a href="${u("/catalogo-v6-8/#productos-v68")}">Volver al catálogo</a><span>/</span><span>${e(brandName(product))}</span></nav>
...
<section class="v65-products v65-related">
  ...
  <section class="v65-grid">${related.map((item) => cardV68(item, origin)).join("")}</section>
</section>
<script type="application/ld+json">${json({
    ...
    {
      bodyClass: "v65 v66 v67 v68 product-detail",
      origin,
      canonicalPath: `/producto-v6-8/${product.slug}/`,
      ogType: "product",
      ogImage: safeImage(product, "detail"),
      homeHref: "/catalogo-v6-8/",
      links: [{ href: "/catalogo-v6-8/#productos-v68", label: "Catálogo" }],
    },
  );
}

export function notFoundPageV68(origin = "http://127.0.0.1:8100") {
  return shell68(
    "No encontrado | Farmagreen Rosario",
    "Producto no encontrado",
    `<section class="empty"><h1>No encontramos ese producto</h1><a href="${u("/catalogo-v6-8/")}">Volver al catálogo</a></section>`,
    { bodyClass: "v65 v66 v67 v68", origin, homeHref: "/catalogo-v6-8/" },
  );
}
...
function pageContext(catalog: Catalog, query: URLSearchParams): PageContext {
  ...
  const metaTitle = brand !== "Todas" ? `${brand} | Catálogo Farmagreen V6.8` : need !== "Todas" ? `${title} | Catálogo Farmagreen V6.8` : q ? `${title} | Farmagreen` : "Farmagreen Rosario | Catálogo V6.8";
...
function filteredProducts(products: Product[], state: QueryState) {
  ...
  return products
    ...
    .filter((product) => {
      ...
      const text = searchTextV68(product);
...
function shell68(title: string, description: string, body: string, options: ShellOptions = {}) {
  const homeHref = options.homeHref || "/catalogo-v6-8/";
  const links = options.links || [];
  const canonicalUrl = options.canonicalPath ? absolute(options.origin || "http://127.0.0.1:8100", options.canonicalPath) : "";
  const canonical = canonicalUrl ? `<link rel="canonical" href="${e(canonicalUrl)}">` : "";
  const ogImage = options.ogImage ? new URL(options.ogImage, options.origin || "http://127.0.0.1:8100").toString() : "";
...
function brandName(product: Partial<Product> | undefined) {
  return String(product?.brand?.name || "Farmagreen");
}

function brandSlug(product: Partial<Product> | undefined) {
  return String(product?.brand?.slug || normalize(brandName(product)));
}

export function sourceImageV68(product: Partial<Product> | undefined, kind: "card" | "detail") {
  return String(product?.images?.[kind] || product?.images?.detail || product?.images?.card || "");
}

export function isPrivateSourceImageV68(value: string) {
  try {
...
function safeImage(product: Partial<Product> | undefined, kind: "card" | "detail") {
  const image = sourceImageV68(product, kind);
  if (!image || !isPrivateSourceImageV68(image)) return image;
...
function productImage(product: Partial<Product>, kind: "card" | "detail", className: string) {
...
type V68Detail = {
  summary?: string[];
  sections?: V68DetailSection[];
};

function productDetail(product: Product) {
  const detail = (product as Product & { detail?: V68Detail }).detail;
  ...
  const sectionsHtml = sections
    ...
    .map((section) => {
      ...
      return `<section class="v68-detail-section v68-detail-${e(section.id)}"><h3>${e(section.title)}</h3>${body}</section>`;
    })
    .join("");
  return `<section class="v68-detail"><h2>Detalle</h2><div class="v68-detail-summary">${summaryHtml}</div>${sectionsHtml}</section>`;
}
