...
const PAGE = 48;
const ROUTE = "/catalogo-v7-beta";
const PDP = "/p-v7-beta/";
const CONTEXT = BOOT.context || {};
const SORT_VALUES = new Set(["relevancia", "posicion", "disponibilidad", "descuento", "precio-asc", "precio-desc", "nombre"]);
...
function catalogFacets(product) {
  const facets = new Map();
  for (const facet of Array.isArray(product?.catalogFacets) ? product.catalogFacets : []) {
    const key = norm(facet?.slug || facet?.name);
    if (!key) continue;
    const current = facets.get(key);
    facets.set(key, {
      slug: facet.slug,
      name: facet.name,
      aliases: [...new Set([...(current?.aliases || []), ...(Array.isArray(facet.aliases) ? facet.aliases : [])])],
...
function productMatchesFacet(product, facetName) {
  const requested = norm(facetName);
  return catalogFacets(product).some(
    (facet) => norm(facet?.name) === requested || (facet?.aliases || []).some((alias) => norm(alias) === requested),
  );
}

function catalogPosition(product, facetName = "Todas") {
  const positions = product?.catalogPositions && typeof product.catalogPositions === "object"
    ? product.catalogPositions
    : {};
  const facets = catalogFacets(product);
  if (facetName !== "Todas") {
    const selected = facets.find((facet) => norm(facet?.name) === norm(facetName));
  ...
  const brandFacet = facets.find(
    (facet) => facet?.kind !== "collection" && norm(facet?.name) === norm(brandName(product)),
  );
...
function blob(product) {
  ...
  return norm(
    [
      ...
      product.line,
      product.barcode,
      ...catalogFacets(product).flatMap((facet) => [facet?.name, ...(facet?.aliases || [])]),
      ...(product.aliases || []),
...
function matches(product, searchIds, hasQuery) {
  ...
  if (S.brand !== "Todas" && !productMatchesFacet(product, S.brand)) return false;
...
function sorted(products) {
  ...
  if (S.sort === "posicion") {
    entries.sort(
      (left, right) =>
        catalogPosition(left.product, S.brand) - catalogPosition(right.product, S.brand) ||
        productTie(left.product, right.product),
    );
  } else if (S.sort === "disponibilidad") {
    entries.sort(
      (left, right) =>
...
function applyParams(params) {
  ...
  const validBrands = new Set([
    "Todas",
    ...S.all.flatMap((product) => catalogFacets(product).map((facet) => facet?.name)).filter(Boolean),
  ]);
...
async function boot() {
  ...
  document.body.dataset.v7BetaCatalogState = "loading";
  ...
  try {
    ...
    document.body.dataset.v7BetaCatalogState = "error";
    return;
  }
  discovery?.removeAttribute("aria-busy");
  ...
  wireFilterMenus();
  render();
  document.body.dataset.v7BetaCatalogState = "ready";
