...
export const GPS_SOURCES_V68 = Object.freeze([
...
export function synchronizeCatalog(
  ...
  sourceResults,
  {
    ...
    expectedSourceIds = GPS_SOURCES_V68.map((source) => String(source.id)),
  } = {},
) {
  if (!baseCatalog || !Array.isArray(baseCatalog.products) || !baseCatalog.products.length) {
    throw new Error("Catálogo base V6.8 inválido o vacío.");
  }
...
export async function runCommercialSync({
  rootDir = ROOT,
  apply = false,
  baseCatalog: providedBaseCatalog,
  sources = GPS_SOURCES_V68,
  fetchHtml = defaultFetchHtml,
  now = () => new Date(),
  minCoverage = environmentNumber("V68_MIN_SYNC_COVERAGE", DEFAULT_MIN_COVERAGE),
  minPriceCoverage = environmentNumber(
    "V68_MIN_PRICE_COVERAGE",
  ...
  const outputPath = path.join(dataDir, "catalog-v68-synced.json");
...
async function defaultFetchHtml(url) {
  return fetchTrustedHtml(url, {
    ...
    timeoutMs: environmentNumber("V68_SYNC_TIMEOUT_MS", 20_000),
    maxAttempts: environmentNumber("V68_SYNC_MAX_ATTEMPTS", 3),
    retryDelayMs: 450,
  });
}
...
function defaultProgress(result) {
  process.stderr.write(
    `[sync-v68] ${result.catalogBrandName}: ${result.products.length} productos, ${result.pages.length} páginas.\n`,
  );
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write(
      "Uso: node scripts/sync-catalog-commerce-v68.mjs [--dry-run|--apply]\n" +
...
if (isMain) {
  main().catch((error) => {
    process.stderr.write(`[sync-v68] ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  });
}
